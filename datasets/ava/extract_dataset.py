...
actionlistfile = "ava_action_list_v2.2.csv"
...
def get_keyframe(videofile, video_id, time_id, outdir_keyframes):
    ...
    ffmpeg_command = 'rm %(outpath)s; \
                      ffmpeg -ss %(timestamp)f -i %(videopath)s  -loglevel error \
...
def visual_bbox(anno_data, action_name, keyfname, video_id, time_id, bbox_ids):
    ...
    for idx in bbox_ids:
        ...
        cv2.putText(frame, action_string[0], pt_to_draw, cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.6, color=[0, 255, 255],
                    thickness=1)
